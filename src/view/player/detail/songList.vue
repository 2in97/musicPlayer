  ...
  <el-container>
    <div class="title-box">
      <el-row class="title-box-nav">
        <el-col :span="6">
          <img :src="songlist.cover" alt="封面" style="height:200px;width:200px" />
          <br />
          <br />
        </el-col>
        <el-col :span="18">
          <div>
            <p class="title-name">{{songlist.lname}}</p>
            <br />
            <h2>{{songlist.artist}}</h2>
            <br />
            <h3 style="color:grey">发行时间 : {{songlist.time}}</h3>
            <br>
            <br>
            <div class="bt">
              <span v-if="!mark">
                ...
                >
                  <el-row>
                    ...
                    <el-col class="col" :span="4">
                      <a @click="insertList(song.id)" class="el-icon-video-play" style="font-size:20px;color: rgb(187, 187, 187);"></a>
                      <a v-if="!marks[index]" @click="likeForSong(song.id,index)" class="icon iconfont icon-dislike" style="font-size:20px;color: rgb(187, 187, 187);padding-left:20px;"></a>
                      <a v-if="marks[index]" @click="dislikeForSong(song.id,index)" class="icon iconfont icon-aixin3" style="font-size:20px;padding-left:20px;"></a>
                      <a @click="addToList(song.id)" class="el-icon-folder-add" style="font-size:20px;color: rgb(187, 187, 187);padding-left:20px"></a>
                    </el-col>
                  </el-row>
                </li>
              </ul>
...
export default {
  ...
  mounted() {
    ...
    this.getSongsByList(1);
    if (sessionStorage.getItem("token") != null) {
      this.isMark(this.lid);
    }
    this.getMySongLists();
  },
  methods: {
      handleCurrentChange_song(val) {
      this.getSongsByList(val);
    },
      changeInnerVisible(){
    ...
    getSongsByList(pageNum) {
       this.$axios
        .get(
          "http://localhost:8080/song/getSongsByList?id=" +
            this.lid +
            "&pageNum=" +
            pageNum
        )
      ...
      this.$axios
        .get("http://localhost:8080/song/getSBLPage?id=" + this.lid)
        .then(res => {
    ...
    like() {
      this.$axios
        .post(
          "http://localhost:8080/user/addToFavoList",
          qs.stringify({
            lid: this.lid,
            tel: sessionStorage.getItem("tel")
          })
    ...
    dislike() {
      this.$axios
        .post(
          "http://localhost:8080/user/deleteFromFavoList",
          qs.stringify({
            lid: this.lid,
            tel: sessionStorage.getItem("tel")
          })
    ...
    isMark(val) {
      this.$axios
        .get(
          "http://localhost:8080/user/listIsMark?lid=" +
            val +
            "&tel=" +
            sessionStorage.getItem("tel")
        )
    ...
    getSongListById() {
      this.$axios
        .get("http://localhost:8080/song/getSongListById?id=" + this.lid)
        .then(res => {
    ...
    detail() {
      this.$router.push({
        name: "artistDetail",
        params: { lid: this.songlist.artist }
      });
    }
  }
};
