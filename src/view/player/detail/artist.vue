  ...
  <el-container>
    <div class="title-box">
      <el-row class="title-box-nav">
        <el-col :span="6">
          <el-avatar :size="160" :src="artist.avatar"></el-avatar>
          <br />
          <br />
        </el-col>
        <el-col :span="18">
          <div>
            <p class="title-name">{{artist.name}}</p>
            <br />
            <h3>{{artist.artist}}</h3>
            <br />
            <br />
      ...
      <div>
        <div class="content-box">
            ...
            <el-col class="col" :span="20">
              <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal">
                <el-menu-item index="1">
                  <a @click="changeShow">单曲</a>
                </el-menu-item>
                <el-menu-item index="2">
                  <a @click="changeShow">专辑</a>
                </el-menu-item>
              </el-menu>
            </el-col>
          </el-row>
          <div v-show="show">
            <div v-if="this.songList.length==0" class="content-box-content">
              <img class="none-img" src="../../../assets/noneBox.png" alt />
              <p class="none-content">没有找到相关内容</p>
            </div>
            <div v-if="this.songList.length!=0">
              <div class="title">
                <el-row>
          ...
          <div v-show="!show">
            <div v-if="this.albumList.length==0" class="content-box-content">
              <img class="none-img" src="../../../assets/noneBox.png" alt />
              <p class="none-content">没有找到相关内容</p>
            </div>
            <div v-if="this.albumList.length!=0" class="list-content-part">
              <ul class="album-ul">
                <li
...
export default {
  ...
  mounted() {
    ...
    this.getArtistById();
    this.getSongsByArtist(1);
    this.getAlbumsByArtist(1);
    this.getMySongLists();
  },
  methods: {
    changeInnerVisible() {
    ...
    handleCurrentChange_song(val) {
      this.getSongsByArtist(val);
    },
    handleCurrentChange_album(val) {
      this.getAlbumsByArtist(val);
    },
    getSongsByArtist(pageNum) {
      this.$axios
        .get(
          "http://localhost:8080/song/getSongsByArtist?id=" +
            this.aid +
            "&pageNum=" +
            pageNum
        )
        .then(res => {
          ...
          for (var i in this.songList) {
            this.isMarkForSong(this.songList[i].id);
          }
        })
        .catch(err => {
          console.log(err);
      ...
      this.$axios
        .get("http://localhost:8080/song/getSBAPage?id=" + this.aid)
        .then(res => {
    ...
    getAlbumsByArtist(pageNum) {
      this.$axios
        .get(
          "http://localhost:8080/album/getAlbumsByArtist?id=" +
            this.aid +
            "&pageNum=" +
            pageNum
        )
      ...
      this.$axios
        .get("http://localhost:8080/album/getABAPage?id=" + this.aid)
        .then(res => {
    ...
    getArtistById() {
      this.$axios
        .get("http://localhost:8080/artist/getArtistById?id=" + this.aid)
        .then(res => {
