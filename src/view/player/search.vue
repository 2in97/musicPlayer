<template>
  <el-container>
    <div class="content-box">
      ...
      <div v-show="type=='2'">
        <div v-if="this.artists.length==0" class="content-box-content">
          <img class="none-img" src="../../assets/noneBox.png" alt />
          <p class="none-content">没有找到相关内容</p>
        </div>
        <div v-if="this.artists.length!=0" class="list-content-part">
          <ul class="list-ul">
            <li v-for="item in artists" :key="item.id" class="li-artist-avater">
              <a @click="artistDetail(item.id)">
                <el-avatar :size="180" :src="item.avatar"></el-avatar>
      ...
      <div v-show="type=='3'">
        ...
        <div v-if="this.albums.length!=0" class="list-content-part">
          <ul class="album-ul">
            <li v-for="item in albums" :key="item.id" style="padding-top:20px">
              <a @click="albumDetail(item.id)">
...
export default {
  ...
  mounted() {
    ...
    if (this.type == "1") this.searchSong(1);
    if (this.type == "2") this.searchArtist(1);
    if (this.type == "3") this.searchAlbum(1);
    this.getMySongLists()
  },
  methods: {
    ...
    searchSong(pageNum) {
      this.$axios
        .post(
          "http://localhost:8080/song/searchSong?search=" +
            this.word +
            "&pageNum=" +
            pageNum
        )
      ...
      this.$axios
        .get("http://localhost:8080/song/getSSongPage?search=" + this.word)
        .then(res => {
    ...
    searchArtist(pageNum) {
      this.$axios
        .post(
          "http://localhost:8080/artist/searchArtist?search=" +
            this.word +
            "&pageNum=" +
            pageNum
        )
      ...
      this.$axios
        .get("http://localhost:8080/artist/getSArtistPage?search=" + this.word)
        .then(res => {
          this.page_artist = res.data;
        })
        .catch(err => {
          console.log(err);
        });
    ...
    searchAlbum(pageNum) {
      this.$axios
        .post(
          "http://localhost:8080/album/searchAlbum?search=" +
            this.word +
            "&pageNum=" +
            pageNum
        )
      ...
      this.$axios
        .get("http://localhost:8080/album/getSAlbumPage?search=" + this.word)
        .then(res => {
